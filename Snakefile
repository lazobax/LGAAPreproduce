...
rule Download_Short_Read:
	...
	conda:
		"sra-tools"
	shell:
		"fasterq-dump -t data/temp --split-files --outdir data/LSCM1/ SRR13558784 SRR13558785 SRR13558792 -p"

rule Download_Long_Read:
	output:
	...
	conda:
		"sra-tools"
	shell:
		"fasterq-dump -t data/temp --outdir data/LSCM1/ SRR13558786 SRR13558788 SRR13558790 SRR13558793 -p"
...
rule LSCM1_Arrange_Assembly_by_RaGOO:
	input:
		...
		ref= "data/ref/LmajorFriedlin_Genome.fasta",
	output:
...
rule LSCM1_Clean_PolishedAssembly_by_funannotate:
	...
	conda:
		"funannotate"
	shell:
		"funannotate clean {params} -i {input} -o {output}"
###
...
checkpoint LSCM1_QC_Assemblies_by_QUAST:
	...
	conda:
		"quast"
	shell:
		"quast.py {params} --threads {threads} --output-dir results/LSCM1_QC_Assemblies_by_QUAST {input} 2>&1 | tee {log}" 
